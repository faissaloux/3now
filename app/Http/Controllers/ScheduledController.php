...
use Stripe\Stripe;
use App\Models\ {
    Chat, PushNotification, Card, Zones, User, Provider, Promocode, ServiceType, UserRequests, RequestFilter, PromocodeUsage, ProviderService, UserRequestRating, UserLocationType, UserComplaint, FareSetting, PeakAndNight, UserRequestPayment, WebRequests
};
//paypal
...
class WebOrderController extends Controller {
    ...
    public function payWithStripe(Request $request) {
        ...
        if ($UserRequest->service_type_id == '19') {
        ...
        if ($UserRequest->service_type_id == '27') {
            $khla = ($kilometer * 1.2) + 7.10;
            $Total = round($khla, 2);
        }
        if ($UserRequest->service_type_id == '32') {
            $Total = ($kilometer * 1.3) + 17;
            $Total = round($Total, 2);
        }
        ...
        if (!empty($UserRequest->promo_code)) {
            if (strtolower($UserRequest->promo_code) == '3nowlaunch2020') {
                $coupon = \DB::table('coupons')->where('user_id', $UserRequest->user_id)->where('coupon', '3nowlaunch2020')->first();
                $data = ['user_id' => $UserRequest->user_id, 'coupon' => '3nowlaunch2020'];
                if (!count((array)$coupon)) {
                    $Total = $Total - 5;
                    \DB::table('coupons')->insert($data);
                }
            }
            if (strtolower($UserRequest->promo_code) == '3now2020') {
                $coupon = \DB::table('coupons')->where('user_id', $UserRequest->user_id)->where('coupon', '3now2020')->first();
                $data = ['user_id' => $UserRequest->user_id, 'coupon' => '3now2020'];
                if (!count((array)$coupon)) {
                    $Total = $Total - 5;
                    \DB::table('coupons')->insert($data);
                }
        ...
        $StripeCharge = $UserRequest->total * 100;
        $Card = Card::where('user_id', Auth::user()->id)->where('is_default', 1)->first();
        Stripe::setApiKey("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");
    ...
    public function initPaymenet(Request $request) {
        $UserRequest = UserRequests::findOrFail($request->id);
    ...
    public function getPricesBasedOnKilomters($kilometer) {
        $prices = ['distance' => $kilometer, 'c-klasse' => round((($kilometer * 1.25) + 15), 2), 'e-klasse' => round((($kilometer * 1.35) + 20), 2), 'vito' => round((($kilometer * 1.40) + 25), 2), 'v-klasse' => round((($kilometer * 1.50) + 30), 2) ];
        return $prices;
    }
    public function getPricesBasedOnKilomtersWithBack($kilometer) {
        $prices = ['distance' => $kilometer, 'c-klasse' => round(((($kilometer * 1.25) + 15) * 2), 2), 'e-klasse' => round(((($kilometer * 1.35) + 20) * 2), 2), 'vito' => round(((($kilometer * 1.40) + 25) * 2), 2), 'v-klasse' => round(((($kilometer * 1.50) + 30) * 2), 2) ];
        return $prices;
    }
    ...
    public function user_update_password(Request $request) {
        $this->validate($request, ['password' => 'required|min:6', 'id' => 'required|numeric|exists:users,id']);
        try {
    ...
    public function getPrices(Request $request) {
        $this->validate($request, ['s_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 'd_longitude' => 'required|numeric', ]);
        try {
    ...
    public function payment(Request $request) {
        $loggedInUser = Auth::loginUsingId(117);
        $this->user_id = Auth::user()->id;
        $this->method = $request->method == 'paypal' ? 'PAYPAL' : 'CARD';
        ...
        $Payment->fixed = $this->total;
        $Payment->distance = $order->distance;
        $Payment->payment_mode = $this->method;
        $Payment->total = $this->total;
        $Payment->save();
        // get the request with full data to send it in email
        $UserRequest = WebRequests::with('user', 'payment', 'provider', 'service_type')->findOrFail($order->id);
        $user = ['email' => $UserRequest->user->email, 'name' => $UserRequest->user->first_name, 'total_fee' => $UserRequest->payment->fixed, 'invoice_id' => $UserRequest->booking_id, 's_address' => $UserRequest->s_address, 'd_address' => $UserRequest->d_address, 'date' => date('d-m-Y'), 'car' => $UserRequest->service_type->name, 's_address' => $UserRequest->s_address, 'd_address' => $UserRequest->d_address];
        ...
        if ($request->method == 'paypal') {
            ...
            $item_1 = new Item();
            $item_1->setName('Item 1')->setCurrency('USD')->setQuantity(1)->setPrice($total_amount);
            ...
            $redirect_urls->setReturnUrl(URL::route('paypal.success'))->setCancelUrl(URL::route('paypal.faild'));
            $payment = new Payment();
        ...
        if ($request->method == 'card') {
            ...
            \Stripe\Charge::create(["amount" => 100 * 100, "currency" => "usd", "source" => $request->stripeToken, "description" => "Test payment from itsolutionstuff.com."]);
        }
    }
    public function iospaypal(Request $request) {
        $total_amount = 10;
        $payer = new Payer();
        $payer->setPaymentMethod('paypal');
        $item_1 = new Item();
    ...
    public function stripePost(Request $request) {
        Stripe::setApiKey(env('STRIPE_SECRET'));
        $charge = \Stripe\Charge::create(["amount" => round($request->input('price')), "currency" => "usd", "source" => $request->input('stripeToken'), "description" => "Test payment from itsolutionstuff.com."]);
        $paid = $charge->jsonSerialize();
        if (isset($paid['id']) && !empty($paid['id'])) {
    ...
    public function later(Request $request) {
        $this->validate($request, ['s_latitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_longitude' => 'required|numeric', 'service_type' => 'required|numeric|exists:service_types,id', 'promo_code' => 'exists:promocodes,promo_code', 'distance' => 'required|numeric', 'use_wallet' => 'numeric', 'payment_mode' => 'required', ]);
        ...
        if (count($szones) > 0) {
            ...
            if (count($dzones) > 0) {
                ...
                if ($request->has('schedule_date') && $request->has('schedule_time')) {
                    ...
                    $CheckScheduling = UserRequests::where('status', 'SCHEDULED')->where('user_id', Auth::user()->id)->whereBetween('schedule_at', [$beforeschedule_time, $afterschedule_time])->count();
                ...
                try {
                    ...
                    $UserRequest->save();
                    $data = array('username' => Auth::user()->first_name, 'usermobile' => Auth::user()->mobile, 'payment_mode' => Auth::user()->payment_mode, 'booking_id' => $booking_id, 'drivername' => $Providers[0]->first_name, 'drivermobile' => $Providers[0]->mobile);
                    ...
                    $this->invoice_new($UserRequest->id);
                    if ($request->ajax()) {
                        return response()->json(['message' => 'New request Created!', 'request_id' => $UserRequest->id, 'price' => 60, 'current_provider' => $UserRequest->provider_id, ]);
                    } else {
                        return response()->json(['message' => 'New request Created!', 'request_id' => "$UserRequest->id", 'price' => 60, 'current_provider' => $UserRequest->provider_id, ]);
    ...
    public function later2(Request $request) {
        try {
            $data = ['s_latitude' => $request->s_latitude, 's_longitude' => $request->s_longitude, 'd_latitude' => $request->d_latitude, 'd_longitude' => $request->d_longitude, 's_address' => $request->s_address, 'd_address' => $request->d_address, 'distance' => $request->distance, 'service_type' => $request->service_type, 'order_date' => $request->schedule_date, 'order_time' => $request->schedule_time, 'kindersitz' => $request->kindersitz, 'babyschale' => $request->babyschale, 'nameschield' => $request->nameschield, 'note' => $request->note,
            ];
            \DB::table('later')->insert($data);
            return response()->json(['message' => 'New request Created!', 'request_id' => $UserRequest->id, 'current_provider' => $UserRequest->provider_id, ]);
    ...
    public function signup(Request $request) {
        $this->validate($request, ['social_unique_id' => ['required_if:login_by,facebook,google', 'unique:users'], 'device_type' => 'required|in:android,ios', 'device_token' => 'required', 'device_id' => 'required', 'login_by' => 'required|in:manual,facebook,google', 'first_name' => 'required|max:255',
        'email' => 'required|email|max:255|unique:users', 'mobile' => 'required', 'password' => 'required|min:6', ]);
        try {
            $User = $request->all();
    ...
    public function change_password(Request $request) {
        $this->validate($request, ['password' => 'required|confirmed|min:6', 'old_password' => 'required', ]);
        $User = Auth::user();
    ...
    public function update_location(Request $request) {
        $this->validate($request, ['latitude' => 'required', 'longitude' => 'required', ]);
    ...
    public function update_profile(Request $request) {
        $this->validate($request, ['first_name' => 'required|max:255', 'last_name' => 'max:255', 'email' => 'email|unique:users,email,' . Auth::user()->id, 'picture' => 'mimes:jpeg,bmp,png', ]);
        try {
    ...
    public function send_schedule(Request $request) {
        $this->validate($request, ['s_latitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_longitude' => 'required|numeric', 'service_type' => 'required|numeric|exists:service_types,id', 'promo_code' => 'exists:promocodes,promo_code', 'distance' => 'required|numeric', 'use_wallet' => 'numeric', 'payment_mode' => 'required|in:CASH,CARD,PAYPAL', 'card_id' => ['required_if:payment_mode,CARD', 'exists:cards,card_id,user_id,' . Auth::user()->id], ]);
        $spoint[0] = $request->s_latitude;
        $spoint[1] = $request->s_longitude;
        $dpoint[0] = $request->d_latitude;
        $dpoint[1] = $request->d_longitude;
        ...
        if (count($szones) > 0) {
            ...
            if (count($dzones) > 0) {
                ...
                $service_type = $request->service_type;
                $promo_code = $request->promo_code;
                $Providers = Provider::with('service')->select(DB::Raw("(6387 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) AS distance"), 'id')->where('status', 'approved')->whereRaw("(6387 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")->whereHas('service', function ($query) use ($service_type) {
                ...
                try {
                    ...
                    if ($request->ajax()) {
                        return response()->json(['message' => 'New request Created!', 'request_id' => $UserRequest->id, 'price' => $khla, 'current_provider' => $UserRequest->provider_id, ]);
                    } else {
                        return redirect('dashboard');
                    }
                }
    ...
    public function invoice($request_id) {
        try {
            ...
            if ($service_type->service_type == '19') {
                $khla = ($kilometer * 1.25) + 25;
                $khla = round($khla, 2);
            }
            if ($service_type->service_type == '27') {
                $khla = ($kilometer * 1.35) + 35;
                $khla = round($khla, 2);
            }
            if ($service_type->service_type == '32') {
                $khla = ($kilometer * 1.40) + 40;
                $khla = round($khla, 2);
            }
            if ($UserRequest->kindersitz > 1) {
                $khla+= ($request->kindersitz * 5);
            }
            if ($UserRequest->babyschale > 1) {
                $khla+= ($request->babyschale * 10);
            }
            if ($UserRequest->nameschield == 'true') {
                $khla+= 15;
            }
            $Total = $khla;
            if ($Total < 0) {
            ...
            $Charge = Charge::create(array("amount" => $StripeCharge, "currency" => "usd", "customer" => Auth::user()->stripe_cust_id, "card" => $Card->card_id, "description" => "Payment Charge for " . Auth::user()->email, "receipt_email" => Auth::user()->email));
    ...
    public function invoice_new($request_id) {
        try {
            $UserRequest = UserRequests::findOrFail($request_id);
            $tax_percentage = Setting::get('tax_percentage', 10);
            $commission_percentage = Setting::get('commission_percentage', 10);
            ...
            //return response()->json(['error' =>'here again 4' .$khla]);
            $Total = $khla;
            if ($Total < 0) {
                $Total = 0.00; // prevent from negative value
    ...
    public function cancel_request(Request $request) {
        $this->validate($request, ['request_id' => 'required|numeric', ]);
        try {
        ...
        try {
            ...
            if (!empty($UserRequests)) {
                ...
                foreach ($UserRequests as $key => $value) {
                    $UserRequests[$key]->static_map = "https://maps.googleapis.com/maps/api/staticmap?" . "autoscale=1" . "&size=320x130" . "&maptype=terrian" . "&format=png" . "&visual_refresh=true" . "&markers=icon:" . $s_map_icon . "%7C" . $value->s_latitude . "," . $value->s_longitude . "&markers=icon:" . $d_map_icon . "%7C" . $value->d_latitude . "," . $value->d_longitude . "&path=color:0x000000|weight:3|enc:" . $value->route_key . "&key=" . env('GOOGLE_MAP_KEY');
                }
            }
            return $UserRequests;
        }
    ...
    public function request_status_check() {
        try {
            ...
            $UserRequests = UserRequests::UserRequestStatusCheck(Auth::user()->id, $check_status)->get()->toArray();
    ...
    public function rate_provider(Request $request) {
        $this->validate($request, ['request_id' => 'required|integer|exists:user_requests,id,user_id,' . Auth::user()->id, 'rating' => 'required|integer|in:1,2,3,4,5', 'comment' => 'max:255', ]);
        $UserRequests = UserRequests::where('id', $request->request_id)->where('status', 'COMPLETED')
        ->where('paid', 0)->first();
        if ($UserRequests) {
            if ($request->ajax()) {
        ...
        try {
            ...
            if ($review == 0) {
                UserRequestRating::create(['provider_id' => $UserRequest->provider_id, 'user_id' => $UserRequest->user_id, 'request_id' => $UserRequest->id, 'user_rating' => $request->rating, 'user_comment' => $request->comment, ]);
            } else {
                $UserRequest->rating->update(['user_rating' => $request->rating, 'user_comment' => $request->comment, ]);
            }
    ...
    public function trips() {
        try {
            ...
            if (!empty($UserRequests)) {
                ...
                $d_map_icon = asset('asset/img/marker-stop.png');
                foreach ($UserRequests as $key => $value) {
                    $UserRequests[$key]->static_map = "https://maps.googleapis.com/maps/api/staticmap?" . "autoscale=1" . "&size=320x130" . "&maptype=terrian" . "&format=png" . "&visual_refresh=true" . "&markers=icon:" . $s_map_icon . "%7C" . $value->s_latitude . "," . $value->s_longitude . "&markers=icon:" . $d_map_icon . "%7C" . $value->d_latitude . "," . $value->d_longitude . "&path=color:0x191919|weight:3|enc:" . $value->route_key . "&key=" . env('GOOGLE_MAP_KEY');
                }
            }
            return $UserRequests;
        }
    ...
    public function estimated_fare(Request $request) {
        $this->validate($request, ['s_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 'd_longitude' => 'required|numeric', 'service_type' => 'required|numeric|exists:service_types,id', ]);
        try {
            ...
            $latitude = $request->s_latitude;
            $longitude = $request->s_longitude;
            $Providers = Provider::whereIn('id', $ActiveProviders)->where('status', 'approved')->whereRaw("(1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")->get();
            $surge = 0;
            ...
            return response()->json(['estimated_fare' => $khla, 'distance' => $kilometer, 'time' => $time,
            'tax_price' => bcdiv($tax_price, 1, 2), 'base_price' => $service_type->fixed, 'wallet_balance' => Auth::user()->wallet_balance, 'discount' => bcdiv($total_discount, 1, 2), 'currency' => $currency]);
        }
        catch(Exception $e) {
            return $e;
            return response()->json(['error' => trans('api.something_went_wrong') ], 500);
    ...
    public function add_promocode(Request $request) {
        $this->validate($request, ['promocode' => 'required|exists:promocodes,promo_code', ]);
        try {
            ...
            if ($find_promo->status == 'EXPIRED' || (date("Y-m-d") > $find_promo->expiration)) {
                if ($request->ajax()) {
                    return response()->json(['message' => trans('api.promocode_expired'), 'code' => 'promocode_expired']);
                } else {
                ...
                if ($request->ajax()) {
                    return response()->json(['message' => trans('api.promocode_already_in_use'), 'code' => 'promocode_already_in_use']);
                } else {
                ...
                $promo->save();
                if ($request->ajax()) {
                    return response()->json(['message' => trans('api.promocode_applied'), 'code' => 'promocode_applied']);
                } else {
    ...
    public function upcoming_trip_details(Request $request) {
        $this->validate($request, ['request_id' => 'required|integer|exists:user_requests,id', ]);
        try {
            $UserRequests = UserRequests::with('payment')->UserUpcomingTripDetails(Auth::user()->id, $request->request_id)->get();
            if (!empty($UserRequests)) {
                $s_map_icon = asset('asset/img/map-start2.png');
                $d_map_icon = asset('asset/img/marker-stop.png');
                foreach ($UserRequests as $key => $value) {
    ...
    public function show_providers(Request $request) {
        ...
        try {
            ...
            if ($request->has('service')) {
                ...
                $Providers = Provider::where('status', 'approved')->whereRaw("(1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")->get();
            }
            if (count($Providers) == 0) {
                if ($request->ajax()) {
    ...
    public function getChat(Request $request) {
        $this->validate($request, ['request_id' => 'required', ]);
        ...
        if ($request->has('message') && $request->has('provider_id') && $request->has('user_id') && $request->has('type')) {
            ...
            $msgCreate = Chat::Create(['request_id' => $request->request_id, 'provider_id' => $request->provider_id, 'user_id' => $request->user_id, 'message' => $request->message, 'type' => $request->type, ]);
        }
    ...
    public function review(Request $request) {
        try {
            $review = UserRequestRating::select('user_rating', 'provider_comment', 'created_at')->where('user_id', Auth::user()->id)->orderBy('id', 'DESC')->get();
