...
class CouponsController extends Controller {
    ...
    public function store(Request $request) {
        ...
        $notification = array('message' => 'Coupon successfully created.', 'alert-type' => 'success');
        return redirect()->route('admin.coupons.home')->with($notification);
    }
    // Update coupon record
    public function update(Request $request, $id) {
        $this->validate($request, ['code' => 'required', 'discount' => 'required|max:3']);
        ...
        $input['code'] = str_replace(' ', '-', strtoupper($input['code']));
        $coupon->update($input);
        $notification = array('message' => 'Coupon successfully updated.', 'alert-type' => 'success');
        return redirect()->route('admin.coupons.home')->with($notification);
    }
    public function delete($id) {
        Coupons::find($id)->delete();
        $notification = array('message' => 'Coupon successfully deleted.', 'alert-type' => 'success');
        return redirect()->route('admin.coupons.home')->with($notification);
    }
    // Apply Coupon
    public function applyCoupon(Request $request) {
        ...
        if ($check) {
            ...
            $notification = array('message' => 'Invalid Coupon!', 'alert-type' => 'error');
            return Redirect()->back()->with($notification);
        }
    }
    // Remove Coupon
    public function removeCoupon() {
        ...
        $notification = array('message' => 'Session Removed!', 'alert-type' => 'success');
        return Redirect()->back()->with($notification);
    }
    //duplicate coupon
