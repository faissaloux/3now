...
use App\Models\ {
    User, Admin, Provider, ServiceType, UserRequests, ProviderService, UserRequestRating, UserRequestPayment, Package, FareSetting, PeakAndNight, ContactUs, Zones
};
use Auth;
use Setting;
use Exception;
...
class AdminController extends Controller {
    ...
    public function settings_store(Request $request) {
        $this->validate($request, ['site_title' => 'required', 'site_icon' => 'mimes:jpeg,jpg,bmp,png|max:5242880', 'site_logo' => 'mimes:jpeg,jpg,bmp,png|max:5242880', ]);
        ...
        if (($request->additional_price >= 0) and ($request->additional_price < 100)) {
            Setting::set('additional_price', $request->additional_price);
        }
        Setting::save();
    ...
    public function settings_payment_store(Request $request) {
        $this->validate($request, ['CARD' => 'in:on', 'CASH' => 'in:on', 'PAYPAL' => 'in:on', 'stripe_secret_key' => 'required_if:CARD,on|max:255', 'stripe_publishable_key' => 'required_if:CARD,on|max:255', 'daily_target' => 'required|integer|min:0',
    ...
    public function pages(Request $request) {
        $this->validate($request, ['page' => 'required|in:page_privacy', 'content' => 'required', ]);
    ...
    public function statement($type = 'individual') {
        try {
            ...
            $revenue = UserRequestPayment::select(\DB::raw('SUM(fixed + distance) as overall, SUM(commision) as commission,SUM(tax) as tax,SUM(discount) as discount'));
            if ($type == 'today') {
    ...
    public function statement_provider() {
        try {
            ...
            foreach ($Providers as $index => $Provider) {
                ...
                $Providers[$index]->payment = UserRequestPayment::whereIn('request_id', $Rides)->select(\DB::raw('SUM(ROUND(fixed) + ROUND(distance)) as overall, SUM(ROUND(commision)) as commission'))->get();
            }
    ...
    public function changeprovidorpassword(Request $request) {
        $this->validate($request, ['password' => 'required|min:6|confirmed', 'password_confirmation' => 'required|min:6']);
        try {
    ...
    public function fare_settings_store(Request $request) {
        // dd($request->all());
        $this->validate($request, ['fare_plan_name' => 'required', 'from_km' => 'required', 'upto_km' => 'required', 'price_per_km' => 'required', 'waiting_price_per_min' => 'required', 'peak_hour' => 'required', 'late_night' => 'required', ]);
        $data = $request->all();
    ...
    public function addpeakAnight(Request $request) {
        ...
        $count = PeakAndNight::where('peak_night_type', $request->peak_night_type)->where('fare_setting_id', $fid)->where('day', $request->day)->count();
        if ($count == 0) {
            $data = ['start_time' => $request->start_time, 'end_time' => $request->end_time, 'day' => $request->day, 'fare_setting_id' => $fid, 'fare_in_percentage' => $request->fare_in_percentage ? : 0, 'peak_night_type' => $request->peak_night_type, //peak or night
            'peak_night_id' => uniqid(), ];
            $res = PeakAndNight::Create($data);
            $all = PeakAndNight::where('peak_night_type', $request->peak_night_type)->where('fare_setting_id', $fid)->get();
    ...
    public function cabAllocation(Request $request) {
        $data = ['fare_setting_id' => $request->fare_setting_id, 'cab_id' => $request->cab_id, 'zone_id' => $request->provider_id, 'description' => $request->description];
    ...
    public function service_update(Request $request, $id) {
        $this->validate($request, ['name' => 'required|max:255', 'capacity' => 'required|numeric', 'fixed' => 'required|numeric', 'distance' => 'required|numeric', 'image' => 'mimes:ico,png,jpeg,jpg']);
        try {
