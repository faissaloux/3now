...
use Session;
use App\Models\ {
    User, Zones, Dispatcher, Provider, UserRequests, RequestFilter, ProviderService, ServiceType, CorporateAccount, Complaint
};
...
class DispatcherController extends Controller {
    ...
    public function cancel_ride(Request $request) {
        ...
        $input = ['dispatcher_id' => Auth::user()->id, 'type' => 'dispatcher'];
    ...
    public function assignCompany(Request $request) {
        $this->validate($request, ['trip_id' => 'required|numeric', 'cab_company_id' => 'required|numeric', ]);
        try {
            $json = array();
    ...
    public function update_trip(Request $request) {
        $this->validate($request, ['first_name' => 'required', 'last_name' => 'required', 'special_note' => 'required', 's_address' => 'required', 'd_address' => 'required', 'email' => 'required', 'mobile' => 'required', 'request_id' => 'required|numeric', 's_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 'd_longitude' => 'required|numeric', ]);
    ...
    public function trips(Request $request) {
        ...
        if (isset($post['filter']) && $post['filter'] != '') {
            ...
            $Trips = UserRequests::whereIn('status', $status)->with('user', 'provider')->orderBy('id', 'desc')->get();
        } else {
            $Trips = UserRequests::with('user', 'provider')->orderBy('id', 'desc')->get();
        }
        return $Trips;
    }
    /**
    ...
    public function assign(Request $request) {
        $this->validate($request, ['provider_id' => 'required|numeric', 'request_id' => 'required|numeric', ]);
        try {
                ...
                if ($Request->status != 'SCHEDULED') {
                    if ($Providers->count()) {
                        ...
                        foreach ($Providers as $Pr) {
                            $inserted_data[] = ['request_id' => $Request->id, 'provider_id' => $Pr->id];
                        }
                        if ($inserted_data) {
            ...
            if ($request->ajax()) {
                ...
                if (Auth::guard('admin')->user()) {
                    ...
                    return redirect()->route('dispatcher.index')->with('flash_success', 'Request Assigned to Provider!');
                }
            }
        }
        catch(Exception $e) {
    ...
    public function store(Request $request) {
        $this->validate($request, ['s_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 'd_longitude' => 'required|numeric', 'service_type' => 'required|numeric|exists:service_types,id', 'distance' => 'required|numeric', ]);
        if ($request->has('booking_type') && $request->booking_type == 2) {
            $this->validate($request, ['amount_customer' => 'required|numeric', ]);
        }
        try {
            ...
            try {
                ...
                $User = User::create(['first_name' => $request->first_name, 'last_name' => $request->last_name, 'email' => $request->email, 'mobile' => $request->mobile, 'password' => bcrypt($request->mobile), 'payment_mode' => 'CASH']);
            }
        }
        //Modified
        ...
        if ($request->has('schedule_time')) {
            try {
                ...
                $CheckScheduling = UserRequests::where('status', 'SCHEDULED')->where('user_id', $User->id)->where('schedule_at', '>', strtotime($request->schedule_time . " - 1 hour"))->where('schedule_at', '<', strtotime($request->schedule_time . " + 1 hour"))->firstOrFail();
                if ($CheckScheduling) {
                    if ($request->ajax()) {
                ...
                if ($UserRequest->status != 'SCHEDULED') {
                    if ($Providers->count()) {
                        ...
                        foreach ($Providers as $Provider) {
                            $inserted_data[] = ['request_id' => $UserRequest->id, 'provider_id' => $Provider->id];
                        }
                        if ($inserted_data) {
                            DB::table('request_filters')->insert($inserted_data);
                        }
    ...
    public function map_ajax(Request $request) {
        try {
            return Provider::with('service')->whereHas('service', function ($query) {
                $query->where('provider_services.status', 'active')->orWhere('provider_services.status', 'riding');
            })->where('latitude', '!=', 0)->where('longitude', '!=', 0)->where('providers.status', 'approved')->get();
        }
        catch(Exception $e) {
            return [];
        }
    ...
    public function getZonesWithProvider() {
        ...
        if ($zones) {
            foreach ($zones as $zone) {
                $drivers = DB::table('providers')->join('provider_zone', 'provider_zone.driver_id', '=', 'providers.id')->join('provider_services', 'provider_services.provider_id', '=', 'providers.id')->select(DB::raw('providers.*, DATE_FORMAT(provider_zone.created_at , "%b %d %h:%i %p") as enter_time, provider_zone.id as driver_position, provider_services.service_number, provider_services.service_model, provider_services.status as provider_status'))->where('provider_zone.zone_id', $zone['id'])->orderBy('provider_zone.id', 'asc')->get()->toArray();
    ...
    public function getAvailableProviders($service_type = 0, $zone_id = 0) {
        ...
        if ($service_type && $zone_id) {
            $Providers = DB::table('providers')->join('provider_zone', 'provider_zone.driver_id', '=', 'providers.id')->join('provider_services', 'provider_services.provider_id', '=', 'providers.id')->select(DB::raw('providers.*, provider_zone.created_at as enter_time, provider_zone.id as driver_position, provider_services.service_number, provider_services.service_model, provider_services.service_type_id, provider_services.status AS provider_current_status'))->where('providers.status', 'approved')->where('provider_services.status', 'active')->where('provider_services.service_type_id', $service_type)->where('provider_zone.zone_id', $zone_id)->orderBy('provider_zone.id')->get();
        } else {
            $Providers = DB::table('providers')->join('provider_zone', 'provider_zone.driver_id', '=', 'providers.id')->join('provider_services', 'provider_services.provider_id', '=', 'providers.id')->select(DB::raw('providers.*, provider_zone.created_at as enter_time, provider_zone.id as driver_position, provider_services.service_number, provider_services.service_model, provider_services.service_type_id, provider_services.status AS provider_current_status'))->where('providers.status', 'approved')->where('provider_services.status', 'active')->orderBy('provider_zone.id')->get();
        }
        return $Providers;
    }
    ...
    function getUserDetail(Request $request) {
        ...
        if ($request->has('id')) {
            if ($u = User::find($request->id)) {
                $user = ['id' => $u->id, 'first_name' => $u->first_name, 'last_name' => $u->last_name, 'email' => $u->email, 'mobile' => $u->mobile];
            }
        }
        return $user;
    }
    ...
    public function openTicket() {
        ...
        return view('dispatcher.open_ticket', compact('data'));
    }
