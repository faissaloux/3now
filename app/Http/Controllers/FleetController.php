...
use Exception;
use App\Models\ {
    User, Fleet, Provider, ServiceType, UserRequests, ProviderService, UserRequestRating, UserRequestPayment
};
...
class FleetController extends Controller {
    ...
    public function dashboard() {
        try {
            $getting_ride = UserRequests::has('user')->whereHas('provider', function ($query) {
            ...
            return view('fleet.dashboard', compact('providers', 'service', 'rides', 'cancel_rides', 'revenue', 'user_cancelled', 'provider_cancelled'));
        }
        catch(Exception $e) {
    ...
    public function map_ajax() {
        try {
            $Providers = Provider::where('latitude', '!=', 0)->where('longitude', '!=', 0)->where('fleet', Auth::user()->id)->with('service')->get();
    ...
    public function profile_update(Request $request) {
        $this->validate($request, ['name' => 'required|max:255', 'company' => 'required|max:255', 'mobile' => 'required|digits_between:6,13', 'logo' => 'mimes:jpeg,jpg,bmp,png|max:5242880', ]);
        try {
    ...
    public function provider_review() {
        try {
            ...
            $Reviews = UserRequestRating::whereIn('request_id', $rides)->where('provider_id', '!=', 0)->with('user', 'provider')->get();
