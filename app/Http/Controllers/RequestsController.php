...
use App\Models\ {
    Zones, Provider, UserRequests, RequestFilter
};
...
class RequestsController extends Controller {
    public function estimated_fare_all(Request $request) {
        ...
        $data = ['s_latitude' => $request->s_latitude, 's_longitude' => $request->s_longitude, 'd_latitude' => $request->d_latitude, 'd_longitude' => $request->d_longitude, 'service_type' => $request->service_type, 'user_id' => Auth::user()->id, 'request_type' => 'instant', ];
    ...
    public function estimated_fare(Request $request) {
        ...
        $result = ['estimated_fare' => $details->price, 'distance' => $details->distance_kilometer, 'time' => $details->distance_minutes_txt, 'currency' => '€', 'service_type' => $request->service_type];
        return response()->json($result);
    }
    public function estimated_fare_validate() {
        return ['s_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 'd_longitude' => 'required|numeric', 'service_type' => 'required|numeric|exists:service_types,id', ];
    }
    public function send_request(Request $request) {
        ...
        $data = ['s_latitude' => $request->s_latitude, 's_longitude' => $request->s_longitude, 'd_latitude' => $request->d_latitude, 'd_longitude' => $request->d_longitude, 'service_type' => $request->service_type, 'coupon' => $request->promo_code, 'user_id' => Auth::user()->id, 'request_type' => 'instant', ];
        ...
        if ((count($szones) > 0) and (count($dzones) > 0)) {
            ...
            return response()->json(['message' => 'New request Created!', 'request_id' => $UserRequest->id, 'price' => $UserRequest->total, 'current_provider' => $UserRequest->provider_id, ]);
        }
    }
    ...
    public function rules() {
        return ['s_latitude' => 'required|numeric', 'd_latitude' => 'required|numeric', 's_longitude' => 'required|numeric', 'd_longitude' => 'required|numeric', 'service_type' => 'required|numeric|exists:service_types,id', 'distance' => 'required|numeric', 'use_wallet' => 'numeric', 'payment_mode' => 'required|in:CASH,CARD,PAYPAL', 'card_id' => ['required_if:payment_mode,CARD', 'exists:cards,card_id,user_id,' . Auth::user()->id], ];
    }
    ...
    public function getProviders($distance, $latitude, $longitude, $service_type) {
        return Provider::with('service')->select(DB::Raw("(6387 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) AS distance"), 'id', 'device_token', 'first_name', 'android_token')->where('status', 'approved')->whereRaw("(6387 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")->whereHas('service', function ($query) use ($service_type) {
    ...
    public function CheckScheduling($schedule_date, $schedule_time) {
        ...
        $CheckScheduling = UserRequests::where('status', 'SCHEDULED')->where('user_id', Auth::user()->id)->whereBetween('schedule_at', [$before, $after])->count();
