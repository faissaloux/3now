...
use \Carbon\Carbon;
use App\Models\ {
    Fleet, Account, Provider, ServiceType, UserRequests, UserRequestPayment
};
class AccountController extends Controller {
    ...
    public function statement($type = 'individual', $request = null) {
        try {
            ...
            $cancel_rides = $cancel_rides->count();
            $revenue = $revenue->get();
            return view('account.providers.statement', compact('rides', 'cancel_rides', 'revenue', 'commision'))->with('page', $page);
        }
        catch(Exception $e) {
            return back()->with('flash_error', 'Something Went Wrong!');
        }
