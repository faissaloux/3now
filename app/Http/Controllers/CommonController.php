...
use Session;
use App\Models\ {
    ServiceType, Provider, ProviderDocument, PushNotification, Page, ContactUs, Complaint, Blog, LostItem, User
};
...
class CommonController extends Controller {
    ...
    public function password_update(Request $request) {
        $this->validate($request, ['email' => 'required|email', 'password' => 'min:6|required_with:password_confirmation|same:password_confirmation', 'password_confirmation' => 'min:6']);
        try {
    ...
    public function complaint_form(Request $request) {
        $json = array();
        $this->validate($request, ['name' => 'required', 'email' => 'required|email', 'transfer' => 'required', 'message' => 'required', ]);
        try {
    ...
    public function sendMessage(Request $request) {
        $json = array();
        $this->validate($request, ['name' => 'required', 'email' => 'required|email', 'subject' => 'required', 'message' => 'required', 'phone' => 'required', ]);
        try {
            $message = new ContactUs();
            $message->name = $request->name;
            $message->email = $request->email;
    ...
    public function lostItemForm(Request $request) {
        $json = array();
        $this->validate($request, ['name' => 'required', 'email' => 'required|email', 'lost_item' => 'required', 'phone' => 'required', ]);
        try {
    ...
    public function driver_story() {
        $providers = DB::table('providers')->join('provider_profiles', 'providers.id', '=', 'provider_profiles.provider_id')->select('providers.*', 'provider_profiles.description')->where('providers.status', 'approved')->whereNotNull('provider_profiles.description')->paginate(4);
    ...
    public function searchingajax() {
        ...
        foreach ($allrequests as $request) {
            ...
            if ($minutes > 2) {
                DB::table('user_requests')->where('status', 'SEARCHING')->update(['status' => "CANCELLED"]);
            }
        }
    }
