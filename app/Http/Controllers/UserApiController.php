...
use App\Models\ {
    Chat, PushNotification, Card, Zones, User, Provider, Promocode, ServiceType, UserRequests, RequestFilter, PromocodeUsage, ProviderService, UserRequestRating, UserLocationType, UserComplaint, FareSetting, PeakAndNight, UserRequestPayment
};
use Stripe\Charge;
use Stripe\Stripe;
...
class UserApiController extends Controller {
    ...
    public function iosToken(Request $request) {
        try {
            $user = \Auth::user();
    ...
    public function tips(Request $request) {
        ...
        if (isset($request->request_id) and is_numeric($request->request_id) and isset($request->tips) and is_numeric($request->tips)) {
            $UserRequest = UserRequests::find($request->request_id);
            $UserRequest->tips = $request->tips;
            $UserRequest->save();
            $data['error'] = '';
    ...
    public function coupon(Request $request) {
        ...
        $check = $coupon_helper->check($request->coupon);
        $valid = $check->valid;
        if ($valid) {
            $data['error'] = '';
            $data['success'] = 'coupon available';
            $data['discount'] = $check->discount_value;
            $data['discount_value'] = $check->discount_value;
            $data['discount_type'] = $check->discount_type;
            $data['maxDiscount'] = $check->maxDiscount;
        } else {
            $data['error'] = 'this coupon not exists';
            $data['success'] = '';
        }
        return response()->json($data);
        $coupon_used = strtolower($request->coupon);
        $coupon = \DB::table('coupons')->where('user_id', \Auth::user()->id)->where('coupon', $request->coupon)->first();
        if (count($coupon) > 0) {
            $data['error'] = 'this coupon already used';
            $data['success'] = '';
        } else {
            if ($coupon_used == '3nowlaunch2020' || $coupon_used == '3now2020') {
                $data['error'] = '';
                $data['success'] = 'coupon available';
                $data['discount'] = '5';
            } else {
    ...
    public function coupon2(Request $request) {
        $verify = (new \App\Helper\CalculateHelper())->verifyCoupon($request->coupon);
        $data = [];
        if ($verify) {
            $data['error'] = '';
            $data['success'] = 'coupon available';
        ...
        $data['error'] = 'this coupon already used or does not exist';
        return response()->json($data);
    }
    public function send_schedule_payment(Request $request) {
        ...
        try {
            $post = array('from' => 'contact@3now.de', 'fromName' => '3now', 'apikey' => '9ECAE3B0E7E28B94621D30D634B0238ACC12BE45DA5CC17DEC385C99EE08C9212403CDE46FE482701696293D221895D8', 'subject' => 'Buchungsbestätigung', 'to' => $user['email'], 'bodyHtml' => view("emails.invoice", ['user' => $user, 'full_order' => $UserRequest]), 'isTransactional' => false);
            $ch = curl_init();
            curl_setopt_array($ch, array(CURLOPT_URL => $url, CURLOPT_POST => true, CURLOPT_POSTFIELDS => $post, CURLOPT_RETURNTRANSFER => true, CURLOPT_HEADER => false, CURLOPT_SSL_VERIFYPEER => false));
            $result = curl_exec($ch);
            curl_close($ch);
        }
        catch(Exception $ex) {
    ...
    public function calculatePrice(Request $request) {
        ...
        if ($request->service_type == '19') {
            $Total = ($kilometer * 1.25) + 25;
            $Total = round($Total, 2);
        }
        if ($request->service_type == '27') {
            $Total = ($kilometer * 1.35) + 35;
            $Total = round($Total, 2);
        }
        if ($request->service_type == '32') {
            $Total = ($kilometer * 1.40) + 40;
            $Total = round($Total, 2);
        }
        if ($request->kindersitz > 1) {
            $Total+= (($request->kindersitz * 5) - 5);
        }
        if ($request->babyschale > 1) {
            $Total+= (($request->babyschale * 10) - 10);
        }
    ...
    public function save_schedule(Request $request) {
        ...
        if (count($szones) > 0) {
            ...
            if (count($dzones) > 0) {
                ...
                try {
                    $details = "https://maps.googleapis.com/maps/api/directions/json?origin=" . $request->s_latitude . "," . $request->s_longitude . "&destination=" . $request->d_latitude . "," . $request->d_longitude . "&mode=driving&key=" . $this->Google_Api_Key;
                    $json = curl($details);
                    ...
                    $UserRequest->provider_id = '289';
                    $UserRequest->current_provider_id = '289';
                    $UserRequest->service_type_id = $request->service_type;
                    $UserRequest->payment_mode = $request->payment_mode;
                    //	$UserRequest->promo_code =	$request->promo_code;
                    $count = \DB::table('coupons')->where('user_id', Auth::user()->id)->whereIn('coupon', ['3nowlaunch2020', '3now2020'])->count();
                    if ($count == 0) {
                        $UserRequest->promo_code = $request->promo_code;
                    }
                    ...
                    $UserRequest->total = $khla;
                    $UserRequest->save();
                    if ($request->ajax()) {
                        $user = ['email' => $UserRequest->user->email, 'name' => $UserRequest->user->first_name, 'total_fee' => $UserRequest->total, 'invoice_id' => $UserRequest->booking_id, 's_address' => $UserRequest->s_address, 'd_address' => $UserRequest->d_address, 'date' => date('d-m-Y'), 'car' => $UserRequest->service_type->name, 's_address' => $UserRequest->s_address, 'd_address' => $UserRequest->d_address, ];
                        // dd($user);
                        $url = 'https://api.elasticemail.com/v2/email/send';
                        try {
                            $post = array('from' => 'contact@3now.de', 'fromName' => '3now', 'apikey' => '9ECAE3B0E7E28B94621D30D634B0238ACC12BE45DA5CC17DEC385C99EE08C9212403CDE46FE482701696293D221895D8', 'subject' => 'Buchungsbestätigung', 'to' => $user['email'], 'bodyHtml' => view("emails.invoice_scheudle_website", ['user' => $user, 'full_order' => $UserRequest]), 'isTransactional' => false);
                            $ch = curl_init();
                            curl_setopt_array($ch, array(CURLOPT_URL => $url, CURLOPT_POST => true, CURLOPT_POSTFIELDS => $post, CURLOPT_RETURNTRANSFER => true, CURLOPT_HEADER => false, CURLOPT_SSL_VERIFYPEER => false));
                            $result = curl_exec($ch);
                            curl_close($ch);
                        ...
                        return response()->json(['message' => 'New request Created!', 'request_id' => $UserRequest->id, 'price' => $khla, ]);
                    } else {
    ...
    public function cancel_request(Request $request) {
        ...
        try {
            ...
            if (in_array($UserRequest->status, ['SEARCHING', 'STARTED', 'ARRIVED', 'SCHEDULED'])) {
                ...
                (new SendPushNotification)->UserCancellRide2($UserRequest);
                if ($request->ajax()) {
    ...
    public function currentProdiverLocation(Request $request) {
        $provider = UserRequests::find($request->request_id)->provider;
    ...
    public function trips() {
        try {
            ...
            if (!empty($UserRequests)) {
                ...
                $d_map_icon = asset('asset/img/marker-stop.png');
                foreach ($UserRequests as $key => $value) {
                    $UserRequests[$key]->static_map = "https://maps.googleapis.com/maps/api/staticmap?" . "autoscale=1" . "&size=320x130" . "&maptype=terrian" . "&format=png" . "&visual_refresh=true" . "&markers=icon:" . $s_map_icon . "%7C" . $value->s_latitude . "," . $value->s_longitude . "&markers=icon:" . $d_map_icon . "%7C" . $value->d_latitude . "," . $value->d_longitude . "&path=color:0x191919|weight:3|enc:" . $value->route_key . "&key=" . $this->Google_Api_Key;
                }
            }
            return $UserRequests;
        }
    ...
    public function estimated_fare_new(Request $request) {
        ...
        try {
            $details = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $request->s_latitude . "," . $request->s_longitude . "&destinations=" . $request->d_latitude . "," . $request->d_longitude . "&mode=driving&sensor=false&key=" . $this->Google_Api_Key;
            $json = curl($details);
            $details = json_decode($json, TRUE);
            ...
            if ($request->service_type == '27') {
                ...
                $price_27 = round($khla, 2);
            }
            if ($request->service_type == '32') {
                $khla = ($kilometer * 1.3) + 17;
                $khla = round($khla, 2);
            }
            $prices = ['19' => $price_19, '27' => $price_27, '32' => $price_32, ];
            return response()->json(['estimated_fare' => $prices, 'distance' => $kilometer, 'time' => $time, 'tax_price' => bcdiv($tax_price, 1, 2), 'base_price' => $service_type->fixed, 'wallet_balance' => Auth::user()->wallet_balance, 'discount' => bcdiv($total_discount, 1, 2), 'currency' => $currency]);
        }
        catch(Exception $e) {
            return $e;
            return response()->json(['error' => trans('api.something_went_wrong') ], 500);
    ...
    public function estimated_fare(Request $request) {
        ...
        try {
            ...
            'tax_price' => bcdiv($tax_price, 1, 2), 'base_price' => $service_type->fixed, 'wallet_balance' => Auth::user()->wallet_balance, 'discount' => bcdiv($total_discount, 1, 2), 'currency' => $currency, 'service_type' => $request->service_type]);
        }
        catch(Exception $e) {
            return $e;
            return response()->json(['error' => trans('api.something_went_wrong') ], 500);
    ...
    public function getChat(Request $request) {
        ...
        if ($request->has('message') && $request->has('provider_id') && $request->has('user_id') && $request->has('type')) {
            ...
            $notification = ['title' => 'chat', 'body' => $request->message, 'icon' => 'ic_message_icon'];
            $extraNotificationData = ["message" => $notification];
            $fcmNotification = ['to' => $order->provider->device->token, //single token
            'notification' => $notification, 'data' => ["msg_type" => "chat", "request_id" => $request->request_id, "image_url" => "image_url", "user_name" => $userName, "msg" => "msg"]];
