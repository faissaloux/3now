...
use Stripe\Stripe;
use App\Models\ {
    User, Provider, UserRequests, UserRequestPayment
};
//paypal
...
class WebOrderController extends Controller {
    ...
    public function stripePost(Request $request) {
        ...
        try {
           \Stripe\Charge::create(["amount" => $UserRequests->total * 100, "currency" => "EUR", "source" => $_POST['my_stripeToken'], "description" => "payment from 3now.de "]);
        }
    ...
    public function SendEmail(Request $request) {
        ...
        $user = ['email' => $UserRequest->emailadress, 'name' => $UserRequest->user->first_name, 'total_fee' => $UserRequest->payment->fixed, 'invoice_id' => $UserRequest->booking_id, 's_address' => $UserRequest->s_address, 'd_address' => $UserRequest->d_address, 'date' => date('d-m-Y'), 'car' => $UserRequest->service_type->name, 's_address' => $UserRequest->s_address, 'd_address' => $UserRequest->d_address, ];
    ...
    public function iospaypal() {
        ...
        $item_1 = new Item();
        $item_1->setName('Item 1')->setCurrency('EUR')->setQuantity(1)->setPrice($total);
