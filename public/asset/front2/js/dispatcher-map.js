...
function initMap() {
	...
	marker = new google.maps.Marker({
        ...
        icon: bases_url+'/asset/img/marker-start.png'
    });
    ...
    markerSecond = new google.maps.Marker({
        ...
        draggable: true,
        anchorPoint: new google.maps.Point(0, -29),
        icon: bases_url+'/asset/img/marker-end.png'
    });
...
function getUpdateFilterData(props) {
	...
	initMap();
	$.get(bases_url+'/ilyft/dispatcher/dispatcher/trips/', {
		 filter: props
	}, function(result) { 
...
function makeListHtml(props, result ) {
	...
	if( data.length > 0 ) {
		for(var i=0; i < data.length; i++) {	
			...
			html += 	'<div class="il-item '+data[i].status+'" id="trip_item_'+data[i].id+'">';
			html += 		'<a class="text-black" data-id="'+ data[i].id +'" href="#">';
			html += 			'<div class="media">';
			html += 				'<div class="media-body" onClick="ongoingInitialize('+data[i].id+')">';
			
			html +=					'<p class="tag_trip">'+ data[i].user.first_name +'<span class="mod_tag_'+data[i].id+' tag pull-right '+$tag+'">'+data[i].status+'</span></p>';
			...
			html +=					'<h6 class="media-heading">Payment: '+ data[i].payment_mode +'</h6>';
			...
			if( data[i].status == 'SEARCHING' || data[i].status == 'ACCEPTED' || data[i].status == 'STARTED' || data[i].status == 'ARRIVED' || data[i].status == 'PICKEDUP' || data[i].status == 'SCHEDULED' || data[i].status == 'PENDING' ) {
				...
				html += 				' | <button type="button" class="btn btn-info btn_action"  request_id="'+data[i].id+'" data-toggle="modal" cancel_status="edit" data-target="#booking-modal">EDIT</button> ';
				...
				if( data[i].status == 'PENDING' ||  data[i].status == 'SCHEDULED' ) {
					...
					html +=   				'| <button type="button" class="btn btn-info" onClick="DispatcherAssignList('+data[i].id+','+1+')">ASSIGN DRIVER</button>';
					
				}
				
				if( data[i].status == 'ACCEPTED' || data[i].status == 'STARTED' || data[i].status == 'ARRIVED' || data[i].status == 'PICKEDUP' ) {	
					html +=   				'| <button type="button" class="btn btn-info" onClick="DispatcherAssignList('+data[i].id+','+2+')">RE-ASSIGN DRIVER</button>';
				}
